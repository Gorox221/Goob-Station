...
public sealed partial class ShipShieldsSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        // Drop bubble if the emitter is gone or the shield state is inconsistent.
        var cleanupQuery = EntityQueryEnumerator<ShipShieldedComponent>();
        while (cleanupQuery.MoveNext(out var gridUid, out var shieldedComp))
        {
            if (shieldedComp.Source is { } sourceUid && !Exists(sourceUid))
            {
                UnshieldEntity(gridUid, shieldedComp);
                continue;
            }
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ShipShieldEmitterComponent, EntityTerminatingEvent>(OnEmitterTerminating);
    ...
    private EntityUid ShieldEntity(EntityUid entity, MapGridComponent? mapGrid = null, EntityUid? source = null)
    {
        ...
        if (source is not null && TryComp<ShipShieldEmitterComponent>(source.Value, out var emitterForTex))
        {
            shieldVisuals.ShieldTexture = emitterForTex.ShieldTexture;
    ...
    /// <summary>
    /// Removes the shield bubble and <see cref="ShipShieldedComponent"/> from the grid. Safe if the bubble was already deleted.
    /// </summary>
    public bool UnshieldEntity(EntityUid uid, ShipShieldedComponent? component = null)
    {
    ...
    /// <summary>
    /// If this emitter created the grid's shield, remove that shield (e.g. when the emitter entity is deleted).
    /// </summary>
    private void TryUnshieldFromEmitter(EntityUid emitterUid, ShipShieldEmitterComponent emitter)
    {
        if (emitter.Shielded is { } gridUid
        ...
        if (Transform(emitterUid).GridUid is { } g
            && TryComp<ShipShieldedComponent>(g, out var sc)
            && sc.Source == emitterUid)
        {
            UnshieldEntity(g, sc);
        }
    }

    private void OnEmitterTerminating(EntityUid uid, ShipShieldEmitterComponent component, ref EntityTerminatingEvent args)
    {
        _pvsSys.RemoveGlobalOverride(uid);
        TryUnshieldFromEmitter(uid, component);
    }
