...
public sealed class ShipShieldOverlay : Overlay
{
    ...
    public ShipShieldOverlay(IEntityManager entityManager, IPrototypeManager prototypeManager, IResourceCache resourceCache)
    {
        ...
        _unshadedShader = prototypeManager.Index(UnshadedId).Instance();
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        while (enumerator.MoveNext(out var uid, out var visuals, out var fixtures, out var xform))
        {
            ...
            var path = string.IsNullOrEmpty(visuals.ShieldTexture)
                ? "/Textures/_Crescent/ShipShields/shieldtex.png"
                : visuals.ShieldTexture;
            Texture texture;
            try
            {
                texture = _resourceCache.GetTexture(path);
            }
            catch
            {
                texture = _resourceCache.GetTexture("/Textures/_Crescent/ShipShields/shieldtex.png");
            }
