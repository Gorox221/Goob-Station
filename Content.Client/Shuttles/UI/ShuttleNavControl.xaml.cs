...
public sealed partial class ShuttleNavControl : BaseShuttleControl
{
    ...
    // CorvaxGoob: biome zone data for radar display (world coordinates)
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        // CorvaxGoob: Draw biome zones on radar
        if (ShowBiomes && _biomeZoneCoords.Length > 0 && _coordinates != null)
        {
            DrawBiomeZonesInternal(handle);
        }
        ...
        if (_consoleEntity != null && xformQuery.TryGetComponent(_consoleEntity, out var consoleXform))
        {
            ...
            {
                var ce = _consoleEntity.Value;
                var consolePositionWorld = _transform.GetWorldPosition(ce);
    ...
    private void DrawBiomeZonesInternal(DrawingHandleScreen handle)
    {
        ...
        var count = _biomeZoneCoords.Length;
        if (count == 0)
            return;

        // Console position in world space
        var consoleWorldPos = _coordinates.Value.Position;
        var consoleRot = _rotation ?? Angle.Zero;
        var ourEntRot = RotateWithEntity && _consoleEntity != null
            ? _transform.GetWorldRotation(_consoleEntity.Value)
            : consoleRot;

        // Build transform from world space to radar view space
        var posMatrix = Matrix3Helpers.CreateTransform(consoleWorldPos, consoleRot);
        var ourEntMatrix = _consoleEntity != null && EntManager.TryGetComponent(_consoleEntity, out TransformComponent? ceXform)
            ? Matrix3Helpers.CreateTransform(_transform.GetWorldPosition(ceXform), ourEntRot)
            : Matrix3Helpers.CreateTransform(consoleWorldPos, consoleRot);

        var shuttleToWorld = Matrix3x2.Multiply(posMatrix, ourEntMatrix);
        Matrix3x2.Invert(shuttleToWorld, out var worldToShuttle);
        var shuttleToView = Matrix3x2.CreateScale(MinimapScale, -MinimapScale)
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            {
                var biomeStart = biomeWorldPos + lines[j];
                var biomeEnd = biomeWorldPos + lines[j + 1];
